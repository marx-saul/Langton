...
const Cell = preload("res://Scene/Cell.tscn")
...
func one_step(ant):
	...
	# move the ant
	var dir_diff = cons.orthogonal_vec2(ant.dir)
	var newx = (ant.x + dir_diff.x) % cell_num_x
	if newx < 0: newx += cell_num_x
	var newy = (ant.y + dir_diff.y) % cell_num_y
...
func simulate():
	...
	while simulating:
		...
		generation_proceeded.emit()
		current_ant_num += 1
		current_ant_num %= ants.size()
		await get_tree().create_timer(1.0 / sim_speed).timeout
